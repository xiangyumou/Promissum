...
- **Token 安全**：API Token 存储在服务端，前端不暴露敏感信息
- **灵活扩展**：支持延长锁定时间（多层加密）
...
#### 后端技术
- **Next.js API Routes**：API 代理层
- **远程加密服务**：独立部署的时间锁加密 API

#### 工具库
- **uuid (v13.0.0)**：生成唯一项目 ID（前端创建后传给 API）
- **TypeScript 5**：类型安全语言

> **注意**：加密逻辑（tlock-js, drand-client）已移至远程服务，本地不再处理
...
**API 客户端模块** (`src/lib/api-client.ts`):
- 封装所有远程 API 调用
- 自动注入 Bearer Token
- 统一错误处理
- Base64 图片转换工具
...
所有本地 API Routes 充当代理，转发请求到远程服务：

| 本地端点 | 远程 API | 功能 |
|---------|---------|------|
| `GET /api/items` | `GET /items` | 获取项目列表 |
| `POST /api/items` | `POST /items` | 创建新项目 |
| `GET /api/items/[id]` | `GET /items/:id` | 获取项目详情 |
| `POST /api/items/[id]/extend` | `POST /items/:id/extend` | 延长锁定 |
| `DELETE /api/items/[id]` | `DELETE /items/:id` | 删除项目 |
