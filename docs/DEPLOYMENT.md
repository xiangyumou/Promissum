...
### Understanding the Difference / 理解差异

Promissum supports two deployment modes:

Promissum 支持两种部署模式：

#### Local Development / 本地开发

For development, the application runs locally while databases run in Docker:

开发时，应用在本地运行，数据库在 Docker 中运行：
...
**Characteristics / 特点**：
- App runs on host machine at port 3000
  应用在宿主机 3000 端口运行
- Database accessible via `localhost:5432`
  数据库通过 `localhost:5432` 访问
...
- All services in Docker containers
  所有服务在 Docker 容器中运行
- App connects to database via service name (`db:5432`)
  应用通过服务名连接数据库 (`db:5432`)
- Isolated and reproducible environment
  隔离且可复现的环境
- Easier horizontal scaling
  更容易横向扩展
- Database ports not exposed externally (security)
  数据库端口不对外暴露（安全考虑）

### Environment Variables / 环境变量

The `.env.example` file provides templates for both scenarios:

`.env.example` 文件为两种场景提供模板：
...
**For PRODUCTION / 生产环境**:
```bash
DATABASE_URL=postgresql://promissum:promissum_password@db:5432/promissum
